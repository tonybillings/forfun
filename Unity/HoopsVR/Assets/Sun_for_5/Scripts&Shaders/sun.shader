	...
	float3 RGBtoHSL(in float3 RGB)
  {
    ...
    return float3(HCV.x, S, L);
        ...
        fixed4 frag (v2f i) : SV_Target
        {
          ...
          float glow =  pow( 2.0-dot(-normalize((mul(_Object2World, float4(0.0,0.0,0.0,1.0))-_WorldSpaceCameraPos)), normalize(mul(_Object2World,float4(i.normal,0)))),(_Contour*4.0+2.0));
