...
BButton::BButton(TFT_TYPE_PTR tft) : m_isPressed(false), BorderThickness(1), Tag(0)
{
    Initialize(tft);
}

BButton::BButton(const BButton& other) : m_isPressed(false)
{
    m_tft = other.m_tft;
...
void BButton::Initialize(TFT_TYPE_PTR tft)
{
    m_tft = tft;
}

void BButton::SetTextPosition(int16_t offsetX, int16_t offsetY)
{
    this->Text.Position = BPosition(this->Position.X + offsetX, this->Position.Y + offsetY);  
}

bool BButton::Update(int16_t x, int16_t y)
{
